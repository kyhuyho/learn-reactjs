...
const HeaderMain = () => {
  ...
  const favoriteCount =
    photos.filter((item) => item.isFavorite === true).length || 0;
  return (
    <div className="flex items-center justify-between p-4 bg-white shadow-md">
      ...
      <div className="flex items-center gap-x-5">
        <span className="relative">
          ...
          <span className="absolute top-0 right-0 flex items-center justify-center w-5 h-5 text-xs text-white translate-x-1/2 bg-pink-500 rounded-full">
            {cartCount}
          </span>
        </span>
        <span className="relative">
